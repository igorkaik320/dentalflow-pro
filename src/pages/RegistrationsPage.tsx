...
import { formatCNPJ, formatCPF, formatCurrency, parseCurrencyInput } from "@/lib/utils";
...
function mapSupplier(row: any): Supplier {
  return { id: row.id, name: row.name || "", cnpj: formatCNPJ(row.cnpj || ""), phone: row.phone || "", email: row.email || "", category: row.category || "", notes: row.notes || "" };
}
...
export default function RegistrationsPage() {
  ...
  const saveProcedure = async () => {
    if (!clinic.id) return toast.error("Clínica não vinculada. Verifique o cadastro da clínica atualizados.");
    if (!procForm.name.trim()) return toast.error("Informe o nome do procedimento.");
  ...
  const saveCategory = async () => {
    if (!clinic.id) return toast.error("Clínica não vinculada. Verifique o cadastro da clínica atualizados.");
    if (!catForm.name.trim()) return toast.error("Informe o nome da categoria.");
